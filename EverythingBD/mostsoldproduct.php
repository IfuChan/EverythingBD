...
$sql= "SELECT x.product_name, count(*) FROM product x JOIN ordernum y ON x.product_id=y.product_id GROUP BY y.product_id ";
$result=$conn->query($sql);
if($result-> num_rows>0){
  while($row=$result->fetch_assoc()){
    echo "<tr><td>".$row["product_name"]."</td><td>".$row["count(*)"]."</td></tr>";
  }
}
else{
  echo"0 result";
